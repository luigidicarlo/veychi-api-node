...
app.get("/stores", [validateToken, storeExists], (req, res) => {
  if (!req.store)
    return res.json(new Response(false, null, { message: msg.userLacksStore }));
...
app.post(
  ...
  async (req, res) => {
    ...
    if (req.store)
      return res.json(
        new Response(false, null, { message: msg.storeUserAlreadyExists })
      );

    if (!errors.isEmpty())
      return res.json(new Response(false, null, errors.array()));
    ...
    try {
      ...
      const result = await Store.findOne({
        $or: [{ name: body.name }, { rut: body.rut }]
      }).catch(err => {
        throw err;
      });
      ...
      if (result)
        return res.json(
          new Response(false, null, { message: msg.storeExists })
        );

      const newStore = new Store(body);

      const store = await newStore.save().catch(err => {
        throw err;
      });
...
app.put(
  ...
  async (req, res) => {
    if (!req.store)
      return res.json(
        new Response(false, null, { message: msg.userLacksStore })
      );
    ...
    try {
      ...
      if (result)
        return res.json(new Response(false, null, { message: storeExists }));
      ...
      if (!updated.nModified)
        return res.json(
          new Response(false, null, { message: msg.storeUpdateFailed })
        );
...
app.delete("/stores", [validateToken, storeExists], async (req, res) => {
  ...
  try {
    ...
    if (!store)
      return res.json(
        new Response(false, null, { message: msg.storeNotFound })
      );

    const deleted = await Store.updateOne(
      { _id: req.store._id, active: true },
      { active: false }
    );
    ...
    if (!deleted.nModified)
      return res.json(
        new Response(false, null, { message: msg.storeAlreadyDisabled })
      );

    await onDisabled(store._id).catch(err => {
      throw err;
    });
    ...
    return res.json(new Response(false, null, error));
  }
});
